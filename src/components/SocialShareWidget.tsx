...
import { getSharingPlatforms } from "@/data/socialMediaProfiles";
...
const THEME_CONFIG = {
  ...
  glass: {
    handle:
      "bg-gradient-to-br from-white/20 to-white/10 hover:from-white/30 hover:to-white/20 backdrop-blur-md",
  ...
  const getVerticalPosition = () => {
    switch (verticalPosition) {
      ...
      case "bottom":
        return "bottom-4";
      default:
        return "top-[30vh] -translate-y-1/2";
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`relative z-10 w-full h-full flex flex-col items-center justify-center ${drawerSizeConfig.gap}`}
        >
          {children}
        </div>
      </div>
...
// Default configurations - using the new data structure
const DEFAULT_PLATFORMS: SocialPlatform[] = getSharingPlatforms().map(
  (platform) => ({
    ...platform,
    icon: (() => {
...
export default function SocialShareWidget({
  url,
  title = "Faith Hope Love Group - Insurance Solutions",
