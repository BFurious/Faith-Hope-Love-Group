...
const CARD_CLASSES = {
    container: "flex flex-col items-center bg-white rounded-3xl p-8 shadow-xl justify-between border border-gray-100 hover:shadow-2xl transition-all duration-300 group cursor-pointer",
    iconContainer: "w-16 h-16 flex items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-purple-600 shadow-lg mb-4",
    ...
    button: "w-full inline-flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 transition-all duration-300 group-hover:scale-105",
    buttonIcon: "h-4 w-4"
} as const
...
const ACTION_HANDLERS = {
    ...
    phone: (target: string) => {
        window.location.href = emailService.getPhoneCallLink(target)
    },
    map: (target: string) => {
...
const ContactCard = ({ item, index, delay, onAction }: {
    item: ContactItem
    index: number
    delay: number
    ...
    return (
        <motion.div
            {...ANIMATION_CONFIG}
            transition={{ ...ANIMATION_CONFIG.transition, delay: delay + index * 0.1 }}
...
export default function ContactInfo({ items, delay = 0 }: ContactInfoProps) {
    const handleContactAction = (item: ContactItem) => {
        ...
        if (handler && item.target) {
            handler(item.target)
        } else if (item.type === 'form') {
            ACTION_HANDLERS.form()
        }
    }
